...
# Эта функция ниже создана для добавления дополнительных блоков в цепочку
    def lab1():
        conn = psycopg2.connect(dbname='lab1', user='postgres', password='qwerty', host='localhost')
        cursor = conn.cursor()
    ...
    def create_block(self, proof, previous_hash):
        ...
        block = {'index': len(self.chain) + 1,
                 ...
                 'computer_name': str(db[1].iloc[len(self.chain) % len(db)]),
                 'gpu': str(db[2].iloc[len(self.chain) % len(db)]),
                 'operation_system': str(db[3].iloc[len(self.chain) % len(db)]),
                 'price': str(db[4].iloc[len(self.chain) % len(db)]),
                 'cpu': str(db[5].iloc[len(self.chain) % len(db)]),
                 'ram': str(db[6].iloc[len(self.chain) % len(db)]),
                 'proof': proof,
...
def mine_block():
    ...
    response = {'message': 'A block is MINED',
                'index': block['index'],
                'computer_name': block['computer_name'],
                'gpu': block['gpu'],
                'operation_system': block['operation_system'],
                'price': block['price'],
                'cpu': block['cpu'],
                'ram': block['ram'],
                'proof': block['proof'],
